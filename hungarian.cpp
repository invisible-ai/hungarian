...
Result Solve(const Matrix& input, MODE mode) {
    ...
    for (k = 0; k < mrank; k++) {
        ...
        col_mate[k] = -1;
        if (verbose)
            fprintf(stderr, "node %d: unmatched row %d\n", t, k);  // NOLINT(cppcoreguidelines-pro-type-vararg)
        unchosen_row[t++] = k;
    row_done:;
    }
    ...
    while (true) {
        if (verbose)
            fprintf(stderr, "Matched %d rows.\n", mrank - t);  // NOLINT(cppcoreguidelines-pro-type-vararg)
        q = 0;
        while (true) {
            while (q < t) {
                        ...
                        if (slack[l]) {
                            ...
                            if (del < slack[l]) {
                                if (del == 0) {
                                    ...
                                    parent_row[l] = k;
                                    if (verbose)
                                        fprintf(stderr,  // NOLINT(cppcoreguidelines-pro-type-vararg)
    ...
    breakthru:
        ...
        if (verbose)
            fprintf(stderr, "Breakthrough at node %d of %d!\n", q, t);  // NOLINT(cppcoreguidelines-pro-type-vararg)
        while (true) {
            int j       = col_mate[k];
            col_mate[k] = l;
            row_mate[l] = k;
    ...
    for (k = 0; k < mrank; k++) {
        for (l = 0; l < mrank; l++) {
            if (result.cost[k][l] < row_dec[k] - col_inc[l])
                return {};
        }
    }
    for (k = 0; k < mrank; k++) {
        l = col_mate[k];
        if (l < 0 || result.cost[k][l] != row_dec[k] - col_inc[l])
            return {};
    }
    k = 0;
    for (l = 0; l < mrank; l++) {
    ...
    if (verbose)
        fprintf(stderr, "Cost is %d\n", cost);  // NOLINT(cppcoreguidelines-pro-type-vararg)
...
void PrintMatrix(const Matrix& m) {
    ...
    for (int i = 0; i < rows; i++) {
        ...
        for (int j = 0; j < cols; j++) {
            fprintf(stderr, "%5d ", m[i][j]);  // NOLINT(cppcoreguidelines-pro-type-vararg)
        }
        fprintf(stderr, "]\n");  // NOLINT(cppcoreguidelines-pro-type-vararg)
    }
    fprintf(stderr, "\n");  // NOLINT(cppcoreguidelines-pro-type-vararg)
